...
library(corrplot)
library(gridExtra)
library(grid)       # Pour ajouter le titre commun
library(openxlsx)
library(reshape2)
```

```{r}
Datagenus <- read.delim2("C:/Users/toshiba/Desktop/M2_stat/Analyse multivarié/TP-20241012/Datagenus.csv", header=TRUE)

Datagenus  = Datagenus %>% select(-forest,-code)
...
```{r}
colonnes_especes <- paste0("gen", 1:27)
# Calcul de l'abondance totale des 27 espèces pour chaque parcelle
Datagenus$abondance_totale <- rowSums(Datagenus[colonnes_especes], na.rm=TRUE)

# Calcul de la densité de peuplement arboré en divisant par la surface
Datagenus$density <- Datagenus$abondance_totale /Datagenus$surface

head(Datagenus[,c('surface','density')])
```

### b)

```{r}
#### Centrage et réduction avant de calculer les interactions
n = length(Datagenus$density)
# Sélection des variables géographiques quantitatives et des indices EVI
variables_geo <- grep("^(altitude|pluvio|lon|lat)", colnames(Datagenus), value=TRUE)
variables_evi <- grep("^evi_", colnames(Datagenus), value=TRUE)

# Centre et réduit les variables en utilisant la variance corrigée (variance de l'échantillon)
Datagenus[,variables_geo] <- scale(Datagenus[variables_geo], center=TRUE, scale=TRUE) #* sqrt(n/(n-1))

Datagenus[,variables_evi] <- scale(Datagenus[variables_evi], center=TRUE, scale=TRUE) #* sqrt(n/(n-1))

####Interaction
# Conversion de la variable geology en indicatrices
Datagenus_geology <- acm.disjonctif(Datagenus["geology"])

# Fusion des nouvelles indicatrices dans le dataframe principal
Datagenus <- cbind(Datagenus, Datagenus_geology)

# Créer les interactions entre les variables géographiques quantitatives et les indicatrices de geology

variables_geology <- colnames(Datagenus_geology)

for (geo_var in variables_geo) {
  for (geo_indic in variables_geology) {
    interaction_name <- paste(geo_var, geo_indic, sep = "_")
    Datagenus[[interaction_name]] <- Datagenus[[geo_var]] * Datagenus[[geo_indic]]
  }
}
...
```{r}
# Reprendre toutes les variables explicatives : géographiques, EVI, geology et interactions
variables_explicatives <- c(variables_geo, variables_evi, variables_geology, colnames(Datagenus)[grepl("_(geology)", colnames(Datagenus))])

# Calculer la matrice de corrélation pour toutes les variables explicatives (y compris les interactions)
matrice_correlation <- cor(Datagenus[, variables_explicatives], use="complete.obs")

# Afficher le graphique de corrélation avec une taille de police réduite
corrplot(matrice_correlation, method="color", tl.cex=0.33)
...
```{r}
# Réaliser l'ACP sur les variables explicatives
acp_resultat <- PCA(Datagenus[, variables_explicatives], scale.unit=FALSE, graph=FALSE)

# Visualiser l'éboulis d'inertie avec fviz_eig
fviz_eig(acp_resultat, addlabels = TRUE, ylim = c(0, 50))

```

On remarque l'apparition d'un coude à la 4 ième composante. Cela nous indique de retenir 4 composantes principales qui capture l'éssentiel de la varaince.

À 4 composantes principales, il ya au moins 65% de la variance expliqué. Cela devrais permettre d'avoir un bon compromis entre le bruit et l'information utile.

#### Modèle RCP

```{r}
# Extraire les premières 4 composantes principales de l'ACP
nb_composantes_retenues <- 4
composantes_retenues <- acp_resultat$ind$coord[, 1:nb_composantes_retenues]

# Créer un dataframe avec la variable cible Y_density et les composantes
modele_data <- data.frame(density = Datagenus$density, composantes_retenues)

# Ajuster la régression linéaire
modele_RCP <- lm(density ~ ., data = modele_data)
...
#### graphique de Y_hat en fonction de Y

```{r}
# Graphique des valeurs observées vs prédites
valeurs_predites <- predict(modele_RCP)
plot(Datagenus$density, valeurs_predites, 
     xlab = "Y_density observé", ylab = "Y_density prédit", 
     main = "Graphique des valeurs observées vs prédites")
abline(0, 1, col = "red")
```

Le graphiqueYYYcontreY\^\chapeau{Y}Y\^montre une dispersion notable des points autour de la ligne de prediction parfaite, indiquant une prédiction approximative de la densité de peuplement. Le modèle semble sous-estimer ou surestimer certaines valeurs, ce qui laisse une variabilité importante non expliquée, cohérente avec le faibleR2R\^2R2(\~0,22).

#### Coefficient des vraibles grace à la RCP

```{r}

# Obtenir les loadings des composantes principales retenues (contribution de chaque variable d'origine)
loadings <- acp_resultat$var$coord[, 1:nb_composantes_retenues]

# Coefficients de régression des composantes principales significatives (sans l'intercept)
coeff_composantes <- coef(modele_RCP)[-1]  # Exclure l'intercept

# Calcul des contributions des variables d'origine
coeff_originaux <- loadings %*% coeff_composantes

# Créer un dataframe avec les noms des variables et leurs coefficients d'origine
coeff_originaux_df <- data.frame(Variable = rownames(loadings), Coefficient_d_origine = as.vector(coeff_originaux))
coeff_originaux_df <- coeff_originaux_df[order(coeff_originaux_df$Coefficient_d_origine, decreasing = TRUE), ]

coeff_originaux_df = coeff_originaux_df  %>% 
  select(Variable,Coefficient_d_origine)

write.xlsx(coeff_originaux_df, file = "coefficients_RCP.xlsx", row.names = FALSE)

# Afficher les premiers résultats
head(coeff_originaux_df)
```
...
# Combiner les variables explicatives et la variable cible dans un dataframe
data_pls <- data.frame(variables_explicatives_data, density = Datagenus$density)

modele_RCP <- pcr(density ~ ., data = data_pls, ncomp = n_composantes_optimal, validation = "CV", segments = 10)

# Extraire le PRESS pour le nombre optimal de composantes dans la RCP simple
press_RCP_simple <- modele_RCP$validation$PRESS[1, n_composantes_optimal]
print(paste("Valeur de PRESS pour RCP simple avec", n_composantes_optimal, "composantes :", press_RCP_simple))
```

#### Transformation logarithmique

```{r}
# Transformation logarithmique de la densité Y_density
Datagenus$log_density <- log(Datagenus$density + 1)  # Ajout de 1 pour éviter log(0)

# Créer un dataframe avec la variable transformée et les composantes principales retenues
modele_data_log <- data.frame(log_density = Datagenus$log_density, composantes_retenues)

# Ajuster un modèle de régression linéaire avec la variable transformée
modele_RCP_log <- lm(log_density ~ ., data = modele_data_log)
summary_modele_log <- summary(modele_RCP_log)

# Afficher le R² du modèle transformé
R2_log <- summary_modele_log$r.squared
print(paste("R² du modèle avec log(density) :", R2_log))

# Graphique des valeurs observées vs prédites pour le modèle transformé
valeurs_predites_log <- predict(modele_RCP_log)
plot(Datagenus$log_density, valeurs_predites_log, 
     xlab = "log(Y_density) observé", ylab = "log(Y_density) prédit", 
     main = "Graphique des valeurs observées vs prédites (log-transformation)",
     xlim = c(0, max(Datagenus$log_density)), ylim = c(0, max(valeurs_predites_log)),
     cex.main = 0.8, cex.lab = 0.8)  # Ajuste la taille de police des titres et labels
abline(0, 1, col = "red")

```

On observe une dispersion légèrement moins prononcé que dans la cas sans transformation.
...
# Coefficients de régression des composantes principales significatives (sans l'intercept)
coeff_composantes <- coef(modele_RCP_log)[-1]  # Exclure l'intercept
...
# Enregistrer le dataframe des coefficients d'origine en fichier Excel
write.xlsx(coeff_originaux_df, file = "coefficients_RCP_log.xlsx", row.names = FALSE)

# Afficher les premiers résultats
print("Les premiers coefficients d'origine du modèle log sont :")
print(head(coeff_originaux_df))
```

#### PRESS RCP simple Log
...
```{r}
# Effectuer les ACP pour chaque thème
variables_explicatives_data = Datagenus[,variables_explicatives]

acp_photosynthese <- PCA(variables_explicatives_data[, variables_evi], scale.unit = FALSE, graph = FALSE)

acp_geographie <- PCA(variables_explicatives_data[,setdiff(colnames(variables_explicatives_data), variables_evi)], scale.unit = FALSE, graph = FALSE)

# Éboulis d'inertie pour l'ACP du thème Photosynthèse
plot_photosynthese <- fviz_eig(acp_photosynthese, addlabels = TRUE, ylim = c(0, 50)) + 
  ggtitle("Éboulis d'inertie - Photosynthèse")

# Éboulis d'inertie pour l'ACP du thème Géographie
plot_geographie <- fviz_eig(acp_geographie, addlabels = TRUE, ylim = c(0, 50)) + 
  ggtitle("Éboulis d'inertie - Géographie")

# Afficher les deux graphiques côte à côte
grid.arrange(plot_photosynthese, plot_geographie, ncol = 2)
```

On remarque l'apparition d'un coude à la 3 ième composante des deux éboulis. Cela nous indique de retenir 3 composantes principales qui capture l'éssentiel de la varaince.

#### Modele RCP sur les composantes des deux thèmes

```{r}
# 3. Sélection des composantes principales significatives pour chaque thème
...
Le résulats n'est pas trop différent de celui de la RCP fait sur la globalité des variables.
...
#### Valeur predite en fonction des valeur observé
...
predictions_PLS <- predict(modele_PLS, ncomp = n_composantes_optimal, newdata = data_pls)

# Calcul du R²
sst <- sum((data_pls$density - mean(data_pls$density))^2)  # Somme totale des carrés
sse <- sum((data_pls$density - predictions_PLS)^2)         # Somme des carrés des erreurs
r_squared <- 1 - (sse / sst)
print(paste("R² pour le modèle PLS avec", n_composantes_optimal, "composantes :", round(r_squared, 4)))
...
#### Graphe de corrélation des varibles avec les composantes pls
...
### Graphique d'ajustement des lasso
...
### coefficient de tous les modèles
...
#### Calcul du Press des deux modèles
